...
export default {
    ...
    mounted() {
        ...
        let aRects = $('#a rect'),
         ...
         yRects = $('#y rect'),
         nRects = $('#n rect'),
         giordano = $('#giordano path'),
         underline1 = $('#underline rect'),
         underline2 = $('#underline polygon'),
         elArray = [Array.from(aRects), Array.from(rRects), Array.from(yRects),Array.from(nRects)];

        elArray.map(pixelGroup => {
            let tl = new TimelineMax();
            tl.staggerFrom(pixelGroup, .5, {
                opacity: 0,
                x:this.random(-40,20),
                y:this.random(-40,20),
                // scale:this.random(4.3,4.5),
                onComplete:this.shimmer
            }, .02)

        })
        let tl = new TimelineMax();
        tl.staggerFrom(giordano,.5,{
          y:40,
          opacity:0,
          delay:1,
